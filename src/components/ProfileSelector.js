    ...
    <View key={index} style={{justifyContent: 'center', padding: dp(5)}}>
      <TouchableOpacity style={selectorStyles.profileButton} onPress={() => props.selector(profile)}>
        <View style={{flex: 1, alignSelf: 'flex-end'}}>
          {(pinsBoolArray[index]) ? <Image source={require('../../assets/lock.png')} tintColor={'white'} resizeMode='contain' style={{maxWidth: dp(25), maxHeight: dp(25)}}/> : null}
        </View>
        <View style={[styles.button_container, {flex: 10, justifyContent: 'center'}]}>
          <Text style={[styles.button_text, {fontSize: dp(24)}]}>{profile}</Text>
        </View> 
      </TouchableOpacity>
    </View>,
  );
