...
export function Pictos() {
  ...
  useEffect(() => {
    if (pressed !== '') {
      ...
      if (!selectedPicto) {
        return;
      } else if (selectedPicto.content.length > 0) {
  ...
  const pathButtons = path.map((picto, index) => (
    <TouchableOpacity
      key={picto.name + index}
      style={pathStyles.button}
      onPress={() => backTo(picto)}>
      <Text style={pathStyles.buttonText}>{picto.name}</Text>
    </TouchableOpacity>
  ));
  ...
  const restartButton = [
    <TouchableOpacity
      key={'start'}
      style={[
        pathStyles.backButton,
        {display: pathNames.length > 0 ? 'display' : 'none'},
      ]}
      onPress={() => reboot()}>
      <Icon name="refresh" size={dp(30)} color="#fff" />
    </TouchableOpacity>,
  ];
  ...
  return (
    <View style={{backgroundColor: '#fff', flex: 1}}>
      <View
        style={[
          pathStyles.container,
          {margin: pathNames.length > 0 ? dp(20) : 0},
        ]}>
      ...
      <ScrollView style={{backgroundColor: '#fff'}} persistentScrollbar={true}>
        <View style={pickerStyles.container}>
          <PictoList
            ...
            list={filtered ? filteredContent : pictoList}
          />
        </View>
      </ScrollView>
      <View
